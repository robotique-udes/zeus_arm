...
class RoboticArm() : 
	...
	def forward_kinematics(self, q):
		...
		# Paramètres DH trouvé à partir du CAD du robot
		DH_par = {
			'd' : 		np.array([ self.l1h,	0,  			0, 			  			0]),
			'r' : 		np.array([ self.l1d,  	self.l2,  		self.l3, 				self.l4]),
			'theta' : 	np.array([ self.q[0], 	(pi/2)-q[1],	self.phi-q[2]-(pi/2),	-q[3]-self.phi]),
		...
		WTT = self.dhs2T(
			r_col = DH_par['r'],
			d_col = DH_par['d'],
			theta_col = DH_par['theta'],
			alpha_col = DH_par['alpha']
		)
